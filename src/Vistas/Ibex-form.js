...
const IbexForm = () => {
    ...
    const fetchTransaction = async (id) => {
        try {
            ...
            if (!info.transaction.isInvoice) {
                document.getElementById("txtamount").removeAttribute("disabled");
            }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (amount <= 0 || formData.fullname.trim() === "" || formData.email.trim() === "") {
            ...
            {loading && (
                <div>
                    <dotlottie-player
                        ...
                        background="transparent" speed="1" className="lottie-animation" loop autoPlay >
                    </dotlottie-player>
                ...
                <div className="form-container">
                    ...
                    <form onSubmit={handleSubmit} className="transaction-form">
                        <div className="form-group">
                            ...
                            <input id="txtamount" type="number" min="0" step="0.01" disabled value={amount} onChange={(e) => setAmount(e.target.value)} />
                        </div>
                        <div className="form-group">
                            <label>Moneda</label>
                        ...
                        <div className="form-group">
                            ...
                            <input type="text" value={formData.fullname} onChange={(e) => setFormData({ ...formData, fullname: e.target.value })} required placeholder="Nombres Apellidos"/>
                        </div>
                        <div className="form-group">
                            <label>Email</label>
                            <input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required placeholder="sucorreo@gmail.com"/>
                        </div>
                        <button type="submit" className="btn-ibex">Continuar</button>
                    </form>
                </div>
            )}  
